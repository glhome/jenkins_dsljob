...
pipelineJob('utilities/windows-image') {
    ...
    parameters {
        ...
        stringParam('BASE_ISO_SHA256', '', 'Optional SHA-256 checksum for the base ISO')
        stringParam('SSU_URL', '', 'URL of the Servicing Stack Update (.msu)')
        stringParam('SSU_SHA256', '', 'Optional SHA-256 checksum for the SSU')
        stringParam('LCU_URL', '', 'URL of the Latest Cumulative Update (.msu)')
        stringParam('LCU_SHA256', '', 'Optional SHA-256 checksum for the LCU')
        stringParam('IMAGE_INDEX', '1', 'install.wim image index to service')
        stringParam('OUTPUT_NAME', 'Windows-Custom', 'Output ISO file name without extension')
        stringParam('AGENT_LABEL', 'windows-image-builder', 'Jenkins agent label')
        booleanParam('KEEP_WORKSPACE', false, 'Keep the image workspace after the build')
    }
